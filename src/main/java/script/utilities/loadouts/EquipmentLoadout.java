...
import script.utilities.Interaction;
import script.utilities.OwnedItems;
import script.utilities.Timing;
...
public class EquipmentLoadout {
  ...
  public boolean fulfilled() {
    ...
    if (!extraEquipmentItems.isEmpty()) {
      ...
      extraEquipmentItems.forEach(
          l -> Logger.log("Name: " + l.getItemName() + ", qty: " + l.getItemQty()));
      return false;
    }
  ...
  public boolean fulfill() {
    ...
    while (end.isAfter(Instant.now())
      ...
      // diff check of LoadoutItems parameter and given Equipment before depositing all - fuck you
      // camal
      List<LoadoutItem> extraEquipmentItems = getExtraItems();
      if (!extraEquipmentItems.isEmpty()) {
        Timing.sleepForDelay();
        if (Bank.depositAllEquipment()) {
          extraEquipmentItems.stream()
              .forEach(
                  i ->
                      Logger.log(
                          "found extra Equipment item: "
                              + i.getItemName()
                              + " in qty: "
                              + i.getItemQty()));
          Timing.sleepForTickDelay();
      ...
      for (LoadoutItem loadoutItem : items) {
        loadoutUnnoted.addItem(
            new LoadoutItem(loadoutItem.getItemName(), loadoutItem.getItemQty()));
      }
        ...
        while (end.isAfter(Instant.now())
          ...
          for (LoadoutItem item : items) {
            ...
            if (action != null) {
              ...
              Logger.log(
                  "Found equipment item, attempting to equip with action: "
                      + invyItem.getName()
                      + ", "
                      + action);
    ...
    Logger.log(
        "120s timeout for EquipmentLoadout.fulfill() occurred - failed to fulfill equipment");
    return false;
  }
}
