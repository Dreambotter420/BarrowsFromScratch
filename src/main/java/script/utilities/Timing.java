...
public class Timing {
    ...
    public static int getSleepDelay() {
        ...
        sleepLength =
                getRandomDelay(sleepWeightedDistribution, sleepMin, sleepMax, sleepDeviation, sleepTarget);
        return delay;
    }
    ...
    public static int getRandomDelay(
            boolean weightedDistribution, int min, int max, int deviation, int target) {
        if (weightedDistribution) {
            ...
            return (int)
                    clamp(
                            (-Math.log(Math.abs(Calculations.nextGaussianRandom(0.0, 1.0)))) * deviation + target,
                            min,
                            max);
        } else {
            ...
            return (int)
                    clamp(
                            Math.round(Calculations.nextGaussianRandom(0.0, 1.0) * deviation + target), min, max);
        }
    }
